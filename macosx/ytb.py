...
def show_notify(title=None):
    subprocess.Popen("/usr/bin/osascript -e 'display notification \"正在加载视频，请稍等...!!\" with title \"web2mpv\"'", shell=True)

def play_video(url):
    show_notify()
    srtname = download_srt(url)
    if srtname:
        subprocess.Popen(["/usr/local/bin/you-get", "-p", "mpv --sub-file=%s" % srtname, url])
    else:
        subprocess.Popen(["/usr/local/bin/you-get", "-p", "mpv", url])
